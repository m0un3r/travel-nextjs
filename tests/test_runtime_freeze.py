...
def test_framer_runtime_synced():
    ...
    src = (ROOT / "cloned_site/framerusercontent").rglob("*.mjs")
    dst = (ROOT / "nextjs_export/public/framerusercontent").rglob("*.mjs")
    ...
    src_all = len([p for p in (ROOT / "cloned_site/framerusercontent").rglob("*") if p.is_file()])
    dst_all = len([p for p in (ROOT / "nextjs_export/public/framerusercontent").rglob("*") if p.is_file()])
...
def test_search_index_synced():
    assert (ROOT / "nextjs_export/public/framerusercontent/sites/4I8jtuL0GyWoXILTgNi7Sz/searchIndex-K522tAX0hnKL.json").exists()
    assert (ROOT / "nextjs_export/public/framerusercontent/sites/4I8jtuL0GyWoXILTgNi7Sz/searchIndex-xdixgZY2ERLE.json").exists()


def test_next_config_static_export():
    txt = (ROOT / "nextjs_export/next.config.mjs").read_text(encoding="utf-8")
...
def test_static_params_routes():
    ...
    html_files = list((ROOT / "cloned_site").rglob("*.html"))
    ...
    app_routes = list((ROOT / "nextjs_export/app").rglob("page.tsx"))
    ...
    found = sum(
        1
        for f in (ROOT / "nextjs_export/app").rglob("page.tsx")
...
def test_layout_injects_framer_runtime():
    txt = (ROOT / "nextjs_export/app/layout.tsx").read_text(encoding="utf-8")
...
def test_globals_no_blanket_opacity():
    css = (ROOT / "nextjs_export/app/globals.css").read_text(encoding="utf-8")
...
def test_no_framer_404_in_build_log():
    candidates = [
        ROOT / "nextjs_export/.next/build.log",
        ROOT / "nextjs_export/build.log",
    ]
    log = ""
    log_path = None
    for c in candidates:
